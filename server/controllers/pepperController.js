...
exports.addPepper = async (req, res) => {
  ...
  const { name, description, imageUrl, origin, color, heatLevel, scoville } = req.body;
...
exports.getPeppers = async (req, res) => {
  try {
    ...
    const peppers = await Pepper.find(query)
      .select('name description imageUrl origin color scoville heatLevel createdAt')
      .sort({ name: 1 });
...
// PUT /api/peppers/:id — manager only
exports.updatePepper = async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
  ...
  try {
    const pepper = await Pepper.findById(req.params.id);
    if (!pepper) return res.status(404).json({ message: 'Pepper not found' });

    const duplicate = await Pepper.findOne({
      _id: { $ne: pepper._id },
      name: { $regex: `^${name.trim()}$`, $options: 'i' },
    });
    if (duplicate) return res.status(409).json({ message: 'A pepper with this name already exists' });

    pepper.name = name.trim();
    pepper.description = description.trim();
    pepper.imageUrl = imageUrl || '';
    pepper.origin = origin || '';
    pepper.color = color || '';
    pepper.scoville = scoville || '';
    pepper.heatLevel = heatLevel || 'Medium';
